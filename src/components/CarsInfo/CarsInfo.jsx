...
export const CarsInfo = ({ img, gallery, color, title, info, features, showcaseThumb, bg, carColor }) => {
  const [selectedColor, setSelectedColor] = useState(carColor[0]);


  const handleColorChange = (index) => {
    setSelectedColor(carColor[index]);
  };

  return (
    <section className="xl:py-20 py-10" style={{ backgroundColor: bg }}>
      <div className="xl:flex container md:px-5 xl:items-center xl:justify-between">
        {/* IMG & COLORS */}
        <div className="xl:flex">
          {/* IMG BADGE */}
          <div className="relative">
            <img className="relative top-9 right-8" src={selectedColor} alt="Car" />
          ...
          {/* COLORS */}
          <div className="flex relative items-center justify-center mb-6 xl:flex-col gap-4">
            {color.map((item, i) => (
              <button
                className="w-7 h-7 rounded-full block mr-3 last:mr-0 xl:mr-0"
